...
<div style={{ marginTop: "50px", padding: "0 10px" }}>
  <h3 style={{ fontWeight: "bold", marginBottom: "20px" }}>Reservation History</h3>
  <div
    style={{
      overflowX: "auto",
      border: "2px solid #ccc",
  ...
  >
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      <thead style={{ backgroundColor: "#f2f2f2" }}>
        <tr>
          <th style={thStyle}>Client Name</th>
          <th style={thStyle}>Service(s)</th>
          <th style={thStyle}>Time</th>
          <th style={thStyle}>Total Price (TND)</th>
          <th style={thStyle}>Status</th>
        </tr>
      </thead>
      <tbody>
        {appointments
          .filter((appt) => appt.status !== "pending") // Only past handled appointments
          .map((appt) => {
            const totalPrice = appt.services.reduce((sum, svc) => {
              const match = serviceData.find((s) => s.service === svc);
            ...
            return (
              <tr key={appt.id}>
                ...
                <td style={tdStyle}>{appt.services.join(", ")}</td>
                <td style={tdStyle}>{appt.time}</td>
                <td style={tdStyle}>{totalPrice.toFixed(2)}</td>
                <td style={{ ...tdStyle, fontWeight: "bold", color: appt.status === "accepted" ? "green" : "red" }}>
                  {appt.status.toUpperCase()}
                </td>
              </tr>
            );
